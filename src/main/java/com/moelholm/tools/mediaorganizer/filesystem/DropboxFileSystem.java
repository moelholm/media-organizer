...
public class DropboxFileSystem implements FileSystem {
  ...
  public boolean existingDirectory(Path pathToTest) {
    try {
      ...
      DropboxFile metaData =
          postToDropboxAndGetResponse("/files/get_metadata", dropBoxRequest, DropboxFile.class);
        ...
        Collections.singletonList(
            (request, bytes, execution) -> {
              request
                  ...
                  .put(
                      "Authorization",
                      Collections.singletonList(String.format("Bearer %s", dropboxAccessToken)));
              request
                  .getHeaders()
  ...
  private <T> T postToDropboxAndGetResponse(String path, Object arg, Class<T> responseType)
    ...
    String resultJsonString =
        createRestTemplate()
            .postForObject(
                String.format("https://api.dropboxapi.com/2%s", path), arg, String.class);

    // Note (!) : this is a workaround for an ...ahem temporary issue... with getting the Java POJO
